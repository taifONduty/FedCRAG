...
def equal_share(past, slots, seed, ranked=False):
    """An equal share of ``slots`` from each list in ``past``, in the order of ``past``: drawn
    at random with a seed per list, or, if ``ranked``, the first items of each list. A short
    list's unused slots pass to the longer ones; leftover slots from rounding go to the
    longest."""
    names = list(past)
    chosen = {}
    for idx, name in enumerate(sorted(names, key=lambda n: (len(past[n]), n))):
        ...
        if ranked:
            chosen[name] = list(past[name][:share])
        else:
...
class ReplayMemory:
    ...
    def refill(self, past, ranked=False):
        """Fill the slots the guard queries leave with an equal share of each earlier
        experience's training queries, in experience order: seeded per experience, or the
        first of each ranked list."""
        self._replay = equal_share(past, self.budget - len(self._reserved), [self.seed], ranked)
